...
The Log4Shell vulnerability (CVE-2021-44228) was a zero-day vulnerability reported in November 2021 in Apache Log4j that allows attackers to execute arbitrary code \cite{log4shellcve}. Apache Log4j is a widely popular logging framework built for Java which enhances log messages with additional diagnostic information, formats messages to a given layout, can write messages to various targets, and perform message filtering \cite{log4j}. 

The Log4Shell vulnerability enabled an attacker who could control log messages or log message parameters to execute arbitrary code when message lookup substitution was enabled. Log4Shell exploits vulnerabilities relating to the Java Naming and Directory Interface (JNDI) and message lookup substitution. The attacker injects a malicious JNDI lookup in an HTTP header which is logged by Log4j. When Log4j parses this string, the message substitution feature will cause the application to perform a JNDI lookup and reach out to the attackers LDAP server. The server then responds with references to malicious Java files, which are then downloaded and executed by the victim machine \cite{blackfog2025Log4Shell}. An example malicious payload is shown in Listing~\ref{lst:log4shell_exploit}.

\begin{lstlisting}[
	language=bash,
	label={lst:log4shell_exploit},
	caption={Example Log4Shell exploit payload}
]
GET /victim-endpoint
Host: <HOST>
User-Agent: ${jndi:ldap://malicious-server.com}
\end{lstlisting}

In Listing~\ref{lst:log4shell_exploit}, the User-Agent is crafted to mimic a message lookup substitution (\$\{...\}). If this is logged by a vulnerable version, Log4j will interpret the \textit{jndi} prefix as an instruction to perform a JNDI lookup at \textit{ldap://malicious-server.com}. When the attacker controlled \textit{malicious-server.com} is queried, it responds with a reference to some malicious Java classes that are then downloaded and executed by the vulnerable server. As the attacker controls which files are downloaded, they can execute arbitrary code on the victim machine.

After it's disclosure, the Log4Shell vulnerability was given a CVSS score of 10 due to the low attack complexity, no privilege requirements, and no user interaction \cite{csrb2022log4shell}. The seriousness of this vulnerability was exacerbated by its scope, with hundreds of millions of devices potentially vulnerable and around 2 million attack attempts per hour at the its height \cite{doll2025unraveling}. Apache released a patch for the vulnerability around 2 weeks after the vulnerability was reported, but took considerably longer to propagate through the software community. Use of Log4j was extensive, and many systems had direct and transitive dependencies that relied on Log4j, complicating the process and causing organizations to have to wait for third parties to complete their patch \cite{doll2025unraveling}. The effect of this vulnerability, and the difficulty organizations faced responding to it, highlight the need for strict supply chain security practices. Even if these practices could not have prevented the vulnerability, practices such as SBOMs and automated incident responses would have dramatically improved the response time, possibly reducing the number of successful exploits.

\subsection{SolarWinds Attack}
The SolarWinds Corporation is an American business that develops software to manage networks, systems, and infrastructure. SolarWinds has a variety of federal, state, and local government contracts, and attackers breached SolarWinds software in a software supply chain attack in 2020. This was one of the most widespread and sophisticated hacking campaigns ever conducted against the US federal government \cite{gao2021solarwind}. Attackers were able to gain access to SolarWinds build system and planted a backdoor, Sunburst, in updates to the Orion software that gave the access to many high-profile government and Fortune 500 targets \cite{anisa2024solarwinds}. Investigations into the incident indicated that the breach likely occurred at least one year prior to discovery, with the attackers making slight modifications to builds in 2019 \cite{kovacas2020solarwinds}. Researchers believe the attack was a long-planned event, with attackers creating command and control infrastructure between the initial breach and deployment of the Sunburst backdoor \cite{slowik2020solarwinds}.

In all, around 18,000 SolarWinds customers were affected by the breach, including US federal government agencies \cite{alkhadra2021solar}. Part of what made this attack so difficult to detect was that it came through legitimate channels. Since the attackers had access to the SolarWinds build system, the malicious updates came through legitimate channels and were deployed in normal software updates. 

\section{Conclusions}
The complexity and distributed nature of the software supply chain make it a key attack vector for threat actors. As the security playing field is asymmetric, a system is only as secure as its weakest link. The nature of modern software development means that most software projects are dependent on many libraries and tools from third parties, complicating security compliance practices. Numerous tools and frameworks have been developed in recent decades to improve the security of the supply chain, including dependency management practices, SBOMs, and zero-trust build pipelines. However, adoption of these has been variable and rigorous industry standards have not been outlined. Recent attacks highlight the implications of successful breaches. Since many tools in the supply chain are used by thousands of projects, the scale of these attacks can be massive, such as the recent Log4Shell vulnerability in the Log4j Java library. Without novel security techniques, these attacks are likely to increase in the future as software tools and infrastructure become more distributed. 
